...
class _EditRecipeState extends State<EditRecipe> {
  ...
  TextEditingController _getIngredientNameController(
      int stepIndex, int ingredientIndex, String initialText) {
  ...
  Widget _buildImagePicker(BuildContext context, EditRecipeViewModel viewModel,
      {int? stepIndex}) {
    ...
    return ValueListenableBuilder<int>(
      ...
      builder: (context, version, _) {
        ...
        if (pathIsValid) {
          ...
          imageDisplayWidget = FutureBuilder<Uint8List>(
            ...
            builder: (context, snapshot) {
              if (snapshot.connectionState == ConnectionState.waiting) {
                ...
                debugPrint(
                    "Error reading image file async '$path': ${snapshot.error}");
                ...
                return Image.memory(
                  ...
                  errorBuilder: (context, error, stackTrace) {
                    debugPrint(
                        "Error displaying image bytes from '$path': $error");
                    return Icon(Icons.broken_image,
                        size: 50, color: Colors.grey[600]);
                  },
                );
          ...
          imageDisplayWidget = Icon(Icons.add_a_photo_outlined,
              size: 50, color: Colors.grey[600]);
        }
              ...
              child: ElevatedButton.icon(
                ...
                onPressed: () async {
                  await viewModel.pickAndProcessImage(
                      stepIndex: stepIndex, name: "$baseName.jpg");
                },
              ),
            ),
          ],
  ...
  Widget _buildRecipeStep(BuildContext context, EditRecipeViewModel viewModel,
      int stepIndex, bool isHandset) {
    ...
    return Card(
      ...
      color: Colors.greenAccent[100]
          ?.withAlpha(90), // Light, slightly transparent green
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const Divider(
                height: 20, thickness: 1), // Add a divider after header
  ...
  Widget _buildStepContent(
      BuildContext context, EditRecipeViewModel viewModel, int stepIndex) {
          ...
          children: [
            ...
            if (ingredients.isEmpty)
              Padding(
                ...
                child: Text(AppLocalizations.of(context)!.noIngredientsForStep,
                    style: TextStyle(
                        fontStyle: FontStyle.italic, color: Colors.grey[600])),
              ),
            ...List.generate(
                ingredients.length,
                (index) => _buildIngredientInput(
                    context, viewModel, stepIndex, index)),
            const SizedBox(height: 10),
            Align(
  ...
  Widget foodEntries(
    ...
    final ingredient =
        viewModel.recipe.steps![stepIndex].ingredients[ingredientIndex];
    ...
    final dropdownKey =
        ValueKey('nutrient_${stepIndex}_${ingredientIndex}_${ingredient.name}');

    return FutureBuilder<List<Nutrient>>(
      key:
          dropdownKey, // This key is crucial! It forces rebuild when name changes
        ...
        return Padding(
          ...
          child: DropdownButtonFormField<int>(
            ...
            onChanged: (int? newValue) {
              if (newValue != null) {
                viewModel.updateIngredientFoodId(
                    stepIndex, ingredientIndex, newValue);
              }
            },
          ),
        );
  ...
  Widget foodFactors(key, index, foodId, viewModel) {
    return Selector<EditRecipeViewModel, (int, int)>(selector: (_, vm) {
      ...
      return FutureBuilder<List<Conversion>>(
          ...
          builder: (context, snapshot) {
            ...
            if (snapshot.hasError) {
              return Padding(
                padding: const EdgeInsets.only(top: 8.0),
                child: Text("Error loading conversions: ${snapshot.error}",
            ...
            if (ingredient.selectedFactorId > 0) {
              final selected = conversions
                  .where((c) => c.id == ingredient.selectedFactorId)
                  .toList();
            ...
            return Padding(
              ...
              child: DropdownButtonFormField<int>(
                ...
                value: ingredient.selectedFactorId > 0
                ...
                items: (conversions
                      ..sort((a, b) =>
                          a.factor.compareTo(b.factor))) // sort by factor
                    .map((c) {
                  return DropdownMenuItem<int>(
                    value: c.id,
                    child: Text(getMeasureName(c),
                        overflow: TextOverflow.ellipsis),
                  );
                }).toList(),
                onChanged: (int? newValue) {
  ...
  Widget _buildIngredientInput(BuildContext context,
      EditRecipeViewModel viewModel, int stepIndex, int ingredientIndex) {
    // Check bounds defensively
    if (stepIndex < 0 ||
        stepIndex >= viewModel.recipe.steps.length ||
        ingredientIndex < 0 ||
        ingredientIndex >=
            viewModel.recipe.steps[stepIndex].ingredients.length) {
      return const SizedBox
          .shrink(); // Handle potential index out of bounds during rebuild race conditions
    }
    final ingredient =
        viewModel.recipe.steps[stepIndex].ingredients[ingredientIndex];
    ...
    return StatefulBuilder(
        key: ValueKey(
            'ingredient_${stepIndex}_$ingredientIndex'), // Add a key for stability
        builder: (BuildContext context, StateSetter setLocalState) {
          final nameController = _getIngredientNameController(
              stepIndex, ingredientIndex, ingredient.name);
          ...
          final shapeController =
              TextEditingController(text: ingredient.shape ?? "");

          nameController.addListener(() {
            viewModel.updateIngredientName(
                stepIndex, ingredientIndex, nameController.text);
          ...
          return Padding(
            ...
            child: Column(children: [
                  ...
                  children: [
                    ...
                    SizedBox(
                      ...
                      child: TextFormField(
                        ...
                        keyboardType: const TextInputType.numberWithOptions(
                            decimal: true),
                        onChanged: (val) => viewModel.updateIngredientQuantity(
                            stepIndex, ingredientIndex, val),
                        decoration: InputDecoration(
                    ...
                    SizedBox(
                      ...
                      child: FormBuilderDropdown(
                        ...
                        items: Unit.values.map((e) {
                          String unitText = e.toString() == ""
                              ? AppLocalizations.of(context)!.unit
                              : e.toString();
                          return DropdownMenuItem(
                              key: UniqueKey(),
                              value: e.toString(),
                              child: Text(formattedUnit(unitText, context)
                                  .toLowerCase()));
                        }).toList(),
                        onChanged: (val) => viewModel.updateIngredientUnit(
                            stepIndex, ingredientIndex, val!),
                        decoration: InputDecoration(
                    ...
                    Expanded(
                      child: TextFormField(
                        ...
                        onChanged: (val) => viewModel.updateIngredientShape(
                            stepIndex, ingredientIndex, val),
                        decoration: InputDecoration(
                    ...
                    IconButton(
                      ...
                      icon: const Icon(Icons.remove_circle_outline,
                          color: Colors.red),
                      onPressed: () => viewModel.removeIngredient(
              ...
              foodFactors(
                  stepIndex, ingredientIndex, ingredient.foodId, viewModel),
            ]),
          );
        });
  }
                ...
                if (context.mounted) {
                  if (saved) {
                    Navigator.of(context)
                        .pop(true); // Leave after successful save
                  } else {
      ...
      builder: (context, snapshot) {
        ...
        if (!viewModel.isInitialized) {
          ...
          return Scaffold(
            ...
            body: Center(
                child: Text(
                    'error: ${snapshot.error ?? 'Initialization failed'}')),
          );
        } else {
          return PopScope(
              canPop: false,
                                ...
                                return DropdownButtonFormField<int>(
                                  ...
                                  items:
                                    ...
                                    return DropdownMenuItem<int>(
                                        value: m,
                                        child: Text(
                                            DateFormat.MMMM(l10n.localeName)
                        ...
                        builder: (context, steps, _) {
                          if (steps.isEmpty) {
                            return Padding(
                              ...
                              child: Text(l10n.noStepsAddedYet,
                                  style: TextStyle(
                          ...
                          return Column(
                            children: List.generate(
                              ...
                              (index) => _buildRecipeStep(
                                  context, viewModel, index, isHandset),
                            ),
                          );
                        },
                      ),
